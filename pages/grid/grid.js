...
Page({
  ...
  loadPage: function () {
    ...
    wx.request({
      url: 'https://gs.jewsoft.com/ASHX/ItemServer.ashx?m=itemList1&t=' + Math.round(Math.random() * 10000),
      method: "POST",
      data: {
        "cno": "", "pid": this.data.pid, "psize": this.data.gssize, "ord": this.data.ord, "txt": that.data.txt, "prop": "",
        "it": "", "fgt": "", "xsk": "", "lpmin": "", "lpmax": "", "wpmin": "",
        "wpmax": "", "swmin": "", "swmax": "", "gwmin": "", "gwmax": ""
      },
      header: {
  ...
  sortType: function(e){
    ...
    console.log(e);
    if (id !== 3){
      this.setData({ ord: id, pid: 1, select_price: false})
      this.loadPage()
    }
  },
  //价格排序
  select_price: function(){
    this.setData({ select_price: !(this.data.select_price)})
  },
